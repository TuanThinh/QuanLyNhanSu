using QuanLyNhanSu.DATA; using QuanLyNhanSu.DATA.AddData; using QuanLyNhanSu.DATA.EditData; using QuanLyNhanSu.ENTITY; using System; using System.Collections.Generic; using System.ComponentModel; using System.Data; using System.Drawing; using System.Linq; using System.Text; using System.Threading.Tasks; using System.Windows.Forms; using System.Data.SqlClient;  namespace QuanLyNhanSu.GUI {     public partial class frmChucVu : Form     {         AddChucvu addChucVu = new AddChucvu();          EditThongTinChucVu editThongTinChucVu = new EditThongTinChucVu();        //  string[] data = { };         //ListViewItem listViewItem = new ListViewItem()           public frmChucVu()         {             InitializeComponent();         }                   public void lockControl()         {             txtMaCV.Enabled = false;             txtTenCV.Enabled = false;             btnHuy.Enabled = false;             btnLuu.Enabled = false;             btnSua.Enabled = false;             btnXoa.Enabled = false;             btnThem.Enabled = true;         }          public void openControl()         {             txtMaCV.Enabled = true;             txtTenCV.Enabled = true;             btnHuy.Enabled = true;             btnLuu.Enabled = true;         }          private void frmChucVu_Load(object sender, EventArgs e)         {             lsvChucVu.Items.Clear();             try             {                 SqlConnection conn = new SqlConnection("Data Source=DESKTOP-NE70A7B\\SQLEXPRESS;Initial Catalog=QuanLyNhanSu;Integrated Security=True");                 SqlCommand com = new SqlCommand("Select *from CHUCVU", conn);                 conn.Open();                 SqlDataReader dr = com.ExecuteReader();                 while (dr.Read())                 {                     addList(dr);                 }             }             catch (Exception ex)             {                  throw;             }                    }          private void btnThem_Click(object sender, EventArgs e)         {             openControl();                       }          private void btnLuu_Click(object sender, EventArgs e)         {             lockControl();             txtMaCV.Focus();             ChucVu chucVu = new ChucVu(txtMaCV.Text, txtTenCV.Text);             if (txtMaCV.Text == null || txtTenCV.Text == null)             {                 MessageBox.Show("Bạn chưa điền đủ thông tin!");             }             else             {                 try                 {                     addChucVu.AddProc(chucVu);                     MessageBox.Show("Lưu Thông tin Thành công!");                 }                 catch (Exception a)                 {                     new Exception("Error: " + a.Message);                     MessageBox.Show("Thêm thông tin không thành công!", "ERROR!!!", MessageBoxButtons.OK, MessageBoxIcon.Error);                 }             }         }          private void btnHuy_Click(object sender, EventArgs e)         {             lockControl();         }          private void lsvChucVu_MouseClick(object sender, MouseEventArgs e)         {             openControl();              btnSua.Enabled = true;              txtMaCV.Text = lsvChucVu.SelectedItems[0].SubItems[0].Text;             txtTenCV.Text = lsvChucVu.SelectedItems[0].SubItems[1].Text;              editThongTinChucVu.MaChucVuCu = txtMaCV.Text;         }          private void btnSua_Click(object sender, EventArgs e)         {             lockControl();              ChucVu chucVu = new ChucVu(txtMaCV.Text, txtTenCV.Text);             try             {                 editThongTinChucVu.ExcuteProc(chucVu);             }             catch (Exception _e)             {                 MessageBox.Show("Sửa thông tin chức vụ không thành công", "ERROR!!!", MessageBoxButtons.OK, MessageBoxIcon.Error);             }         }         private void addList(SqlDataReader dr)
        {
